...
export default function RekapitulasiMatrixPage() {
  ...
  const getStatusColor = (status: string) => {
    ...
    switch (s) { case "SAKIT": return "bg-blue-300 text-blue-800"; case "IZIN": return "bg-yellow-300 text-yellow-800"; case "ALPHA": return "bg-red-400 text-white font-bold"; case "HADIR": return "bg-green-50 text-green-600"; default: return "bg-gray-100 text-gray-400"; }
  };

  const getStatusLetter = (status: string) => {
    if (!status) return "";
    const s = status.toUpperCase().trim();
    switch (s) { case "SAKIT": return "S"; case "IZIN": return "I"; case "ALPHA": return "A"; case "HADIR": return "H"; default: return "?"; }
  };
        ...
        <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-0 flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 w-full pt-2 border-t border-gray-100">
             ...
             <div className="flex flex-wrap sm:flex-nowrap w-full sm:w-auto gap-1.5">
               ...
               <button onClick={() => setStatusFilter("TIDAK HADIR")} className={`py-1.5 px-3 text-[11px] font-bold rounded border ${statusFilter === "TIDAK HADIR" ? "bg-purple-600 text-white border-purple-600" : "bg-white text-purple-600 border-purple-200 hover:bg-purple-50"}`}>TIDAK HADIR</button>
            ...
            <TableHeader className="bg-gray-50 sticky top-0 z-10">
              <TableRow>
                ...
                {mode === "BULANAN" ? (
                  <>
                    {daysArray.map(day => (
                      <TableHead key={`h-${day}`} className="w-[28px] p-0 text-center border-r border-gray-200 px-1 text-[10px] sm:text-xs">{day}</TableHead>
                    ))}
                    <TableHead className="text-center font-bold border-l border-gray-200 bg-gray-100 min-w-[50px]">Tot</TableHead>
                  </>
                ) : (
                ...
                displayedData.map((siswa, idx) => {
                  ...
                  return (
                    <TableRow key={siswa.nis} className={`hover:bg-gray-50/50 ${isCritical ? 'bg-red-50/20' : ''}`}>
                      <TableCell className="text-center text-muted-foreground border-r border-gray-200 py-2">{idx + 1}</TableCell>
                      ...
                      {mode === "BULANAN" ? (
                        <>
                          {daysArray.map(day => (
                            <TableCell key={`c-${siswa.nis}-${day}`} className={`p-0 border-r border-gray-200 text-center align-middle border-b-0`}>
